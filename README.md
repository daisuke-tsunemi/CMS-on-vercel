...
上記のような拡張開発、および **microCMS / Next.js を使ったサイト・業務システムの開発全般**を承っています。

- テンプレートをベースにした自社向けカスタマイズ
- 認証・権限管理、入力機能、外部サービス連携の追加
- 既存サイトの microCMS 移行、Jamstack 化
- Web サイト制作、業務システム開発、保守運用

「どこから手を付けるべきか」「この要件は現実的か」といった段階のご相談でも構いません。

**▶ [お問い合わせはこちら](https://soushin-lab.co.jp/contact)**
